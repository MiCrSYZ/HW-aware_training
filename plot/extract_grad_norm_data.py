"""
提取所有noise_boundary文件夹中的grad_norm、grad_norm_std和grad_var数据
"""
import os
import csv
import json
...
def read_metrics_csv(csv_path):
    """读取metrics.csv文件，返回grad_norm、grad_norm_std和grad_var的列表"""
    try:
        grad_norms = []
        grad_norm_stds = []
        grad_vars = []
        ...
        return grad_norms, grad_norm_stds, grad_vars
...
def collect_all_data():
    ...
    for noise_folder in NOISE_FOLDERS:
        ...
        for subfolder in folder_path.iterdir():
            ...
            # 读取metrics数据
            grad_norms, grad_norm_stds, grad_vars = read_metrics_csv(metrics_file)
            ...
            avg_grad_var = sum(grad_vars) / len(grad_vars) if grad_vars else None
            ...
            # 计算grad_var的标准差
            if grad_vars and len(grad_vars) > 1:
                mean_grad_var = avg_grad_var
                variance_grad_var = sum((x - mean_grad_var) ** 2 for x in grad_vars) / len(grad_vars)
                std_grad_var = variance_grad_var ** 0.5
            else:
                std_grad_var = 0.0 if grad_vars else None
            ...
            # 添加grad_var相关数据
            if grad_vars:
                data_item['avg_grad_var'] = avg_grad_var
                data_item['std_grad_var'] = std_grad_var
                data_item['grad_var_all'] = grad_vars
...
def main():
    print("开始提取grad_norm、grad_norm_std和grad_var数据...")
